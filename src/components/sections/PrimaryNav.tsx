...
const PrimaryNav: React.FC = () => {
  ...
  const currentRoute =
    typeof window !== "undefined"
      ? extractRouteFromURL(window.location.href)
      : "";
  ...
  return (
    <nav className="sticky top-0 z-40 bg-white shadow-lg border-b border-gray-200 transition-all duration-300">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 lg:h-20">
          ...
          >
            <span
              className={`block w-6 h-0.5 transition-all duration-300 transform origin-center ${
                toggle
                  ? "rotate-45 translate-y-1 bg-gray-800"
                  : "rotate-0 bg-gray-800"
              }`}
            ></span>
            <span
              className={`block w-6 h-0.5 transition-all duration-300 ${
                toggle ? "opacity-0" : "opacity-100 bg-gray-800"
              }`}
            ></span>
            <span
              className={`block w-6 h-0.5 transition-all duration-300 transform origin-center ${
      ...
      <div
        className={`lg:hidden absolute top-full left-0 w-full bg-white border-t border-gray-200 shadow-2xl transform transition-all duration-300 ${
          toggle
        ...
        <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
          {PrimaryNavbar.map((item: NavItem, index: number) => (
            <Link
              key={item.id}
              href={`/${item.link}`}
              className={`block text-xl font-medium text-gray-800 hover:text-primary-green transition-all duration-300 transform ${
          ...
          <div className="pt-4">
            <Link href="https://play.google.com/store/apps" target="_blank">
              <button
                className={`w-full bg-gradient-to-r from-[rgba(231,177,12,1)] to-[rgba(202,156,15,1)] text-white px-6 py-4 rounded-full font-semibold flex items-center justify-center space-x-2 hover:from-[rgba(202,156,15,1)] hover:to-[rgba(231,177,12,1)] transition-all duration-300 shadow-lg transform ${
                  toggle
